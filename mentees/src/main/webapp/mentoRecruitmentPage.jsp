...
<body>
  ...
  <div class="mentoArea">
    <button class="registerMento" onclick="registMento();">신청</button>
    ...
    <table>
        ...
        <c:forEach items="${mentorList}" var="list">
          <tr>
            ...
            <td width="300px" align="center" class="line_limit"><a href="<c:url value='readRecruit?id=${ list.id }'/>">${list.title }</a></td>
        	...
        	<ul>
                             <!-- 2. 이전버튼 활성화 여부 --> 
                 <c:if test="${page.prev }">
                     <li><a href="mentrecruit?pageNum=${page.start - 1 }&amount=${page.amount}">이전</a></li>
                     </c:if>
                       <!-- 1. 페이지번호 처리 --> 
                      <c:forEach var="num" begin="${page.start }" end="${page.end }">
                      <li class = "paging" class="${page.pageNum eq num ? 'active' : '' }">
                      <a href="mentrecruit?pageNum=${num }&amount=${page.amount}">${num }</a></li>
                     </c:forEach>
                  
                     <!-- 3. 다음버튼 활성화 여부 --> 
                     <c:if test="${page.next }">
                       <li><a href="mentrecruit?pageNum=${page.end + 1 }&amount=${page.amount}">다음</a></li>
                     </c:if>
              </ul>
        </div>
    </div>

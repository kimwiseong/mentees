...
<section class="info">
  ...
  <h3 class="writer">작성자: <a class="nameHover href="<c:url value='searchMember?name=${ mentor.name }'/>"> ${mentor.name }</a></h3>
  <h3 class="mentoTitle"> 제목 : [${mentor.subject}]${mentor.title }</h3>
  
  <h3 class="content">
  </h3>
  <div class="contentInfo">${mentor.content }</div>
  ...
  <form action="checkMentor" method="post">
    <input type="hidden" name="name" value="${mentor.name} "/>
      ...
      <input type="hidden" name="subject" value="${mentor.subject} "/>
                ...
                <tr>
                    <td colspan="5" align="center">
                         <ul>
                             <!-- 2. 이전버튼 활성화 여부 --> 
                             <c:if test="${page.prev }">
                                 <li><a href="readRecruit?id=${mentor.id }&pageNum=${page.start - 1 }">이전</a></li>
                             </c:if>
                                                               
                                <!-- 1. 페이지번호 처리 --> 
                                <c:forEach var="num" begin="${page.start }" end="${page.end }">
                                    <li  class="${page.pageNum eq num ? 'active' : '' }">
                                    <a href="readRecruit?id=${mentor.id }&pageNum=${num }">${num }</a></li>
                                </c:forEach>
                                
                                <!— 3. 다음버튼 활성화 여부 —> 
                                <c:if test="${page.next }">
                                    <li><a href="readRecruit?id=${mentor.id }&pageNum=${page.end + 1 }">다음</a></li>
                                </c:if>
                        </ul>
                    </td>
                </tr>

...
/**
 * Servlet implementation class DeleteRecruitController
 */
@WebServlet("/deleteRecruit")
public class DeleteRecruitController extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final MemberService memberService = new MemberService();
	private static final MentorService mentorService = new MentorService();

    public DeleteRecruitController() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (email == null) {
			...
			String mentorName = mentorService.findNameById(id);
			boolean isDelete = false;
			
			if (memberName.equals(mentorName)) 
				isDelete = mentorService.deleteById(id);

			if (isDelete) {
				out.print("<script>alert('삭제했습니다.'); location.href='mentrecruit'; </script>");
